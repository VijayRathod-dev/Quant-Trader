...
def validate_api_keys(bright_data_key: str, gemini_key: str) -> tuple:
...
def create_sidebar():
    ...
    gemini_api = st.sidebar.text_input(
        "🤖 Gemini API Key",
        type="password",
        help="Get your API key from Gemini platform",
        placeholder="sk-...",
    )
    ...
    **Powered by**: LangChain + Bright Data + Gemini API
    """
    )

    return analyze_button, bright_data_api, gemini_api, analysis_type, custom_query
...
async def run_analysis(
    bright_data_api: str, gemini_api: str, analysis_type: str, custom_query: str
):
    """Run the stock analysis asynchronously"""
    try:
        # Initialize the system
        system = StockResearchSystem(bright_data_api, gemini_api)
...
def main():
    ...
    analyze_button, bright_data_api, gemini_api, analysis_type, custom_query = (
        create_sidebar()
    )
    ...
    if analyze_button:
        # Validate inputs
        is_valid, errors = validate_api_keys(bright_data_api, gemini_api)
        ...
        with st.status(
            ...
            try:
                ...
                results = asyncio.run(
                    run_analysis(
                        bright_data_api, gemini_api, analysis_type, custom_query
                    )
                )
